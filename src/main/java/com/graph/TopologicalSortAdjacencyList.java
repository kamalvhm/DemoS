...
public class TopologicalSortAdjacencyList {
  ...
  public static Integer[] dagShortestPath(Map<Integer, List<Edge>> graph, int start, int numNodes) {
    ...
    dist[start] = 0;
    ///This approach will start from 'starting' node and update nbr weigth in dist table non reachable will be skipped
    for (int i = 0; i < numNodes; i++) {

      int nodeIndex = topsort[i];
      if (dist[nodeIndex] != null) { //This is to start from 'start' node rest all will skip
        List<Edge> adjacentEdges = graph.get(nodeIndex);
        if (adjacentEdges != null) {
          for (Edge edge : adjacentEdges) {

            int newDist = dist[nodeIndex] + edge.weight;//exiting node wsf +edge wiegth
            if (dist[edge.to] == null) dist[edge.to] = newDist; //edge.to is like nbr
